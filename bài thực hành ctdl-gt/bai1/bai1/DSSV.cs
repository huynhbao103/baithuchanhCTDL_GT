...
namespace buoi1
{
    class DSSV
    {
        ...
        // Sắp xếp danh sách với tham số k = 1.theo mã SV, 2.theo tên, 3.theo nghành, 4. Điểm TB,5.theo chuyên ngành,6.điểm tb, 7.học lại.
        ...
        public void DsChuyenNghanh(string cn)
        {
            var svTheoCN = lst.Where(sv => sv.Chuyennganh == cn);
            foreach (var sv in svTheoCN)
            {
                Console.WriteLine(sv.Maso + '\t' + sv.Holot + '\t' + sv.Ten + '\t'
                    + sv.Ngaysinh + '\t' + sv.Phai + '\t' + sv.Chuyennganh);
            }
        }

        public float DiemAVG()
        {
            return lst.Average(sv => sv.DiemTB);
        }

        public void DsHoclai()
        {
            var svHocLai = lst.Where(sv => sv.DiemTB < 5);
            foreach (var sv in svHocLai)
            {
                Console.WriteLine(sv.Maso + '\t' + sv.Holot + '\t' + sv.Ten + '\t'
                    + sv.Ngaysinh + '\t' + sv.Phai + '\t' + sv.Chuyennganh);
            }
